...
test('every Live section is titled the way the Learn tabs are', async () => {
  ...
  try {
    ...
    const SECTIONS = [['tuner', '#secTuner', 'Tuner'], ['play', '#secFind', 'Play'],
...
/* ------------------------------------------------------------------
   THE CONSOLE. The game's card is an instrument panel, not a document:
...
test('the game console is one fixed height per prompt mode, and fits the screen', async () => {
  for (const vp of CONSOLE_VIEWPORTS) {
    const app = await openApp(SILENT, '/index.html#play', vp);
    try {
      const { page } = app;
      await page.click('#startBtn');
...
test('the map dims what this stage does not ask, and can light up your own misses', async () => {
  ...
  try {
    ...
    await page.evaluate(() => {
      const st = JSON.parse(localStorage.getItem('bassTrainer.gamemem.v1') || '{}');
      st.stats = Object.assign({ heat: {}, noteRecent: {} }, st.stats);
      st.stats.heat['A:5'] = 3;
      localStorage.setItem('bassTrainer.gamemem.v1', JSON.stringify(st));
